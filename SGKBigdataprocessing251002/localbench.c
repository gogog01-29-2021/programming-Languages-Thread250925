//build gcc -O2 -std=c11 -pthread lock_bench.c -o lock_bench
//run
# lock = nolock | spin | tas | ttas | backoff
# threads = 1,2,4,8,16,32 ...
# cs_iters = work inside the critical section (0, 50, 500, ...)

./lock_bench tas     8   0
./lock_bench ttas    16  50
./lock_bench backoff 32  500
./lock_bench spin    8   0
./lock_bench nolock  8   0     # expect WRONG sum (data race)
...
static void nl_lock(void *s, unsigned cs){ (void)s; (void)cs; }
...
static void sp_lock(void *s, unsigned cs){ (void)cs; pthread_spin_lock(&((spin_t*)s)->l); }
...
static void tas_lock(void *s, unsigned cs){
    (void)cs;
    tas_t *t = (tas_t*)s;
...
static void ttas_lock(void *s, unsigned cs){
    (void)cs;
    ttas_t *t = (ttas_t*)s;
    for(;;){
...
// ----------------- Backoff (TTAS + exponential backoff) -----------------
typedef struct {
    atomic_bool state;
    unsigned min_wait;
    unsigned max_wait;
...
static void bo_lock(void *s, unsigned cs){
    (void)cs;
    backoff_t *b = (backoff_t*)s;
...
static inline void critical_work(unsigned iters){
    // Pure compute; keeps the lock held
...
// Worker: acquire lock, take next number, add it to sum, release
static void* worker(void *arg){
    arg_t *A = (arg_t*)arg;
    bench_t *B = A->B;

    for(;;){
        B->vt->lock(B->impl, B->cs_iters);
...
int main(int argc, char **argv){
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <nolock|spin|tas|ttas|backoff> <threads> [cs_iters]\n", argv[0]);
        return 1;
    }
    const char *kind = argv[1];
    int T = atoi(argv[2]);
    unsigned cs_iters = (argc >= 4) ? (unsigned)atoi(argv[3]) : 0;

    lock_vtable *vt = pick_vt(kind);
    void *impl = alloc_lock_storage(kind);
    vt->init(impl);
    ...
    arg_t *args = (arg_t*)malloc(sizeof(arg_t)*T);

    double t0 = now_sec();
    for (int i=0;i<T;i++){
        args[i].B = &B; args[i].tid = i;
    ...
    printf("lock=%s threads=%d cs_iters=%u  time=%.3f s  sum=%llu  %s\n",
           vt->name, T, cs_iters, (t1-t0),
           (unsigned long long)B.sum,
           ok ? "[OK]" : "[WRONG]");

    vt->destroy(impl);
    free(impl); free(th); free(args);
    return ok ? 0 : 3;
}
